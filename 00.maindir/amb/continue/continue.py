...
def main_call(st,w_c,d_cp):
    ...
    if st == num:
        for s,f in seed_folder.items():
            ...
            cp_file(os.path.join(my_dir,st,f),'md.rst.%s' % s, \
                    os.path.join(my_dir,nextnum),'00.rst')
            if os.path.isfile(os.path.join(my_dir,nextnum,'00.rst'))==True:
                print 'COOR VEL in place'
                for path in glob(os.path.join(my_dir,'%s/*/*.rst.*' % st)):
