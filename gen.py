...
def asmd():
    def a_work_dir():
        ...
        w.reg_exp(subdir)
    def call_a_Struc(ng,mol,env,workdir,jobdir):
        s = a_Struc_Dirs(ng,mol,env,workdir,jobdir)
        s.a_makeStrucDir()
    def call_a_Smd(ng,mol,env,v,zc,workdir,jobdir):
        config = super_pickle(int(v))
        stages = len(config[int(v)][0][5])
        print 'ASMD is ready with',ng,'for',mol,'in',env,'at velocity',v, \
              'assembled inside',jobdir+'.'
        f = a_Smd_Method(ng,mol,env,v,ts,zc,langevD,sf,workdir,jobdir,gate,\
          ppn_env[env],comp,wt_env[env],q_env[env],howmany,stages,direct,dist,config)
        f.a_makeEnvDir()
        f.a_makeVelDir()
        f.a_makeContainDir()
        f.a_savepickle()
    ...
    a_work_dir()
    [call_a_Struc(ng,mol,env,workdir,jobdir) for ng in ngn for mol in molec \
         for env in environ]
    [call_a_Smd(ng,mol,env,v,envdist[env],workdir,jobdir) for ng in ngn \
         for mol in molec for env in environ for v in n]
